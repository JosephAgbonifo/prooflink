...
export default function CreateProjectPage() {
  ...
  const {
    ...
    defaultValues: { paymentType: "fundraising", currency: "C2FLR" },
  });
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setMessage(null); // Clear the error message once they pick a file
    }
  };
  ...
  const onSubmit = async (formData: CreateProjectForm) => {
    ...
    try {
      ...
      // Check the STATE variable instead of the REF
      if (selectedFile) {
        const uploadedUrl = await uploadToCloudinary(selectedFile);
        if (!uploadedUrl) throw new Error("Image upload failed");
        finalImageUrl = uploadedUrl;
      } else {
        // If selectedFile is null, show the error
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12 md:py-24">
      ...
      <input
        type="file"
        id="project-image-upload" // Use an ID
        accept="image/*"
        ref={fileInputRef}
        onChange={(e) => {
          handleImageChange(e);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-12">
        ...
        <div className="lg:col-span-3">
          ...
          >
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <select
                        ...
                        className="w-full px-5 py-4 bg-Fslate-50 border-2 border-transparent focus:border-moss focus:bg-white rounded-2xl outline-none transition-all font-bold text-slate-900 cursor-pointer appearance-none"
                      >
                        <option value="C2FLR">C2FLR</option>
            ...
            {/* Sidebar / Preview */}
            <div className="lg:col-span-2 space-y-8 md:hidden block">
              {/* Image Upload Area */}
              <label
                htmlFor="project-image-upload"
                className="bg-slate-50 p-8 rounded-[3rem] w-80 h-80 border-2 border-dashed border-slate-200 hover:border-moss transition-colors group relative overflow-hidden flex items-center justify-center cursor-pointer block"
              >
                {previewUrl ? (
                  <img
                    src={previewUrl}
              ...
              {submittedProject && (
                <div className="bg-moss p-8 rounded-[2.5rem] text-white shadow-xl animate-in zoom-in-95 duration-500">
                  ...
                  <p className="text-xs font-medium leading-relaxed">
                    Supporters can now contribute using this ID on Quirklr.
                  </p>
                </div>
              )}
            </div>
